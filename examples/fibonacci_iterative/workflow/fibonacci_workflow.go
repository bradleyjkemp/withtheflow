...
func createReturnResult(dest *int64) func(result []byte, _ withtheflow.FlowHandle) ([]byte, error) {
	return func(result []byte, _ withtheflow.FlowHandle) ([]byte, error) {
		resultData := &fibonacci.Result{}
...
func CreateWorkflow(index int64, dest *int64) withtheflow.Workflow {
	flowHandlers := map[string](withtheflow.FlowHandler){
		FibStep:   createFlowStep(index),
		InitStep:  initial,
		PrintStep: createReturnResult(dest),
	}
